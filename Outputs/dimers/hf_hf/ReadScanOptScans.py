...
nameog='hf_hf_linear_mp2_avtz_rab_aahb_opts'
file.close()
titleline=" Title:"
energyline=" E2 =   "
reline=" !       Rah "
...
while i<len(donor):
    ...
    if line[:len(khbline)]==khbline:
        ka=float(line.split()[3])*mDynepA_to_EhpA2
    if line[:len(freqline)] == freqline:
        wa = float(line.split()[2])
    i+=1
eb=0
i=0
...
while True:
    ...
    while i<len(lines):
        ...
        if line[:len(khbline)]==khbline:
            khbtemp=float(line.split()[-1])*mDynepA_to_EhpA2
        if line[:len(freqline)]==freqline:
            whbtemp=float(line.split()[-1])
        if line[:len(saveline)]==saveline:
            i+=1
            try: line=lines[i][:-1]
            except: break
