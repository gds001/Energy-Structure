...
data=np.genfromtxt("h2o_h2o_linear_mp2_avtz_rahs_rab.csv",delimiter=',',dtype=np.double).T
...
s=1e-12
file=open("../../SemiRigidScans_Simpler_Relax_rb_positive_dipole_deriv/h2o_h2o/Fits_H2O_H2O_Polar.json",'r')
...
while True:
    try:
        file=open("Fits_H2O_H2O_Polar.json",'r')
        dic=json.loads(file.read())
    except:
    ...
    file=open("Fits_H2O_H2O_Polar.json",'w')
    file.write(str)
    file.close()
