...
filesvals=["../Outputs/dimers/nh3_nh3/nh3_nh3_linear_mp2_avtz_rab_aahb_opts.csv",
       ...
       "../Outputs/dimers/hf_h2o/hf_h2o_mp2_avtz_rab_aahb_opts.csv",
       "../Outputs/dimers/hf_nh3/hf_nh3_mp2_avtz_rab_aahb_opts.csv"]
params=[]
for i in range(6):
    try:
        file=open(files[i],'r')
...
for i in range(6):
    data=np.genfromtxt(filesvals[i],delimiter=',').T
    data=data[:,data[0]>=Res[i]]
    data=data[:,data[0]<Res[i]+3]
    data=data[:,np.abs(data[1])<=15]
    s=1e-12
    print(files[i])
    print(Res[i])
    ps = params[i][type]['x']
    ...
    psPolar=np.copy(ps)
    psPolar[0]=0
    if i==1:pcm=axes[k,j].scatter(data[3]*100,data[2]*627.5,marker='.',c=data[1],cmap='twilight')
    else: axes[k,j].scatter(data[3]*100,data[2]*627.5,marker='.',c=data[1],cmap='twilight')
    ...
    axes[k, j].plot(rs * 100, (ka*ps[6]*rs-x*rs**2-y*rs**3-z*rs**4
                               ...
                               +C/ro*rs+B/ri*rs
                               ) * 627.5, c='tab:gray',zorder=-100)
    """print(ka*ps[6]*rs-x*rs**2-y*rs**3-z*rs**4
                               +np.log((rs+ro)**C/(ri-rs)**B*ri**B/ro**C)
                               -C/ro*rs-B/ri*rs
                               )"""
    ...
    axes[k,j].text(np.amax(dr) * 105 * 0.95, np.amin(Ess) * 1.1 * 0.075,
                   "MAE: {:.2f}\nkcal/mol".format(RMSD(data[2],ans)*627.5),
                   va='bottom',ha='right',fontsize=10)
    axes[k,j].set_title(titles[i],fontsize=12)
    axes[k,j].set_ylim(0,np.amax(Ess)*627.5*1.1)
    axes[k,j].set_xlim(0,np.amax(dr)*105)
...
cbar=fig.colorbar(pcm,ax=axes[:,2],shrink=0.75,label='$\\theta$ (deg.)')
...
plt.savefig("../Figures/EeVsdr0_nonlinear_{}.png".format(type),dpi=600)
plt.show()
